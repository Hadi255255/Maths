...
var newExam_errors_mode = 0; let newExam = document.getElementById('new');
...
division_arabic.onclick = function () {
    ...
    plus2.style.display = minus2.style.display =
        side_container1.style.display = side_container.style.display = sum_div.style.display = 'none';
...
home_arabic.onclick = function () {
    ...
    if (arabic_numbers_home == true) { ar_arabic_choose_home.onclick() }
    else { arabic_choose_home.onclick() };
    setTimeout(() => {
        window.onload();
    }, 20);
};
        ...
        if (count2.length != 0) {
            ...
            if (((answer.value != '' || num1.value == '') && (page != 'g'))
                ...
                if (count == count1.length * count2.length && count != 0) {
                    ...
                    if (mark < 50) { level = "ضعيف" }
                    else if (mark < 60) { level = "وسط" }
                    else if (mark < 70) { level = "جيد" }
                    else if (mark < 80) { level = "جيد جداً" }
                    else if (mark < 95) { level = "ممتاز" }
                    else if (mark != 100 && mark >= 95) { level = "ممتاز جداً" }
                    else if (mark == 100) {
                        level = "ممتاز جداً";
                        if (on_effects == 0) {
                            wenner.muted = false;
...
newExam_errors.onclick = function () {
    ...
    pm = 1;
    if (errors_num1.length != 0) {
        scroll_result3(); scroll_result4(); scroll_result5();
        start_errors.disabled = false;
        start.innerHTML = "اضغط: ابدأ اختبار الأخطاء ↙ ";
        start_errors.style.animation = "starterrors 100ms 10 linear";
        if (window.innerWidth < 1100) {
...
function english() {
    ...
    if (arabic_choose.checked) { arabic(); }
        ...
        num1.value = eng_num1; num2.value = eng_num2;
    }
    answer.addEventListener('input', function () {
        for (let i = 0; i < answer.value.length; i++) {
            if ((numbers.includes(answer.value[i]) == false || num1.value == '') && page != 'g') {
...
function arabic_count_errors() {
    ...
    if (mode == 'cycle_2') {
        ...
        if (arabic_choose.checked) {
            newExam_errors.innerHTML =
                `<pre class='retest'>أعد اختبار الأخطاء (ctrl)</pre>عدد الأخطاء: <span class='number_errors'> ${count_errors_arabic}</span>`;
        }
        else {
            newExam_errors.innerHTML =
                `<pre class='retest'>أعد اختبار الأخطاء (ctrl)</pre>عدد الأخطاء: <span class='number_errors'> ${len}</span>`;
        }
    };
}
arabic_count_errors();
...
minus1.addEventListener('click', function () {
    if (sound.checked == true) {
        ...
        if (page != 'g') { answer.onmouseover() }
        else if (page == 'g') { num1.onmouseover() }
...
ar_for_who_p.onclick = ar_how_p.onclick = ar_multipication_p.onclick = ar_division_p.onclick = ar_general_tests_p.onclick = ar_end_p.onclick = function () {
    window.scroll({ left: 500, top: 0, behavior: "smooth" });
